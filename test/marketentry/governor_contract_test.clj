  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Systems LB"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Systems LB" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest beneficial-ownership-undisclosed-is-held-and-unoverridable
  (testing "undisclosed beneficial-ownership information -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:beneficial-ownership-undisclosed} (-> (store/ledger db) last :basis)))
