(ns marketentry.marketentryllm
  ...
  the LBN public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "beneficial-ownership-declared?=" (:beneficial-ownership-declared? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-beneficial-ownership-declaration? e))
                              (:beneficial-ownership-declared? e))
